...
ATACSEQ_DATA_FILE_PATH="../../ATACSeq/ATACSeq_analysis_q10/"
ANNOTATION_FILE_PATH = "../../ChIPseq/CSV_feature_files_mm10/"
...
# ANNOTATION_FILE_PATH = "../../ChIPseq/CSV_feature_files_mm10/" # path for deature annotation genomic coordinates
...
def analyseFeatureCoverage(analysis, FeatureAnnotationFile, FeatureName, interval=(-5050,5050), chromosome=None):
    ...
    specification and strand="-" the data is inverted to provide the correct orientation relative to start and end. If no "strand" column is given, unstranded feature will be assumed.
    If the FeatureAnnotationFile does not contain an "end" columns the feature will be treated as a position and an "end" column equal to the "start" columns will be inserted.
    
    If FeatureAnnotationFile does not contain "chr" and "start" columns it is treated as a gene list with the index as gene names. For each gene the postition of the TSS is used as
    feature position and the TSS interval will be ploted.

    The analysis can be restricted by specifying a chromosome as either "chrX" or "autosome".

    """
    Feature_Annotation=pd.DataFrame.from_csv(FeatureAnnotationFile)
    if "chr" in Feature_Annotation.columns and "start" in Feature_Annotation.columns:
        if not "end" in Feature_Annotation.columns:
            Feature_Annotation["end"] = Feature_Annotation["start"]
        if chromosome == "chrX":
    ...
    else:  # generate TSS for gene list in Feature_Annotation with index as gene names
        featDF = analysis.Annotation[analysis.Annotation.index.isin(Feature_Annotation.index)]
        if chromosome == "chrX":
            FeatureDF = featDF.loc[ featDF["genomic_interval"].apply(lambda x: x.chrom.upper()=="CHRX")]
        else:
            if chromosome.lower()[:7] == "autosom":
                FeatureDF = featDF.loc[ featDF["genomic_interval"].apply(lambda x: x.chrom.upper()[:4]!="CHRX" and x.chrom.upper()[:3]=="CHR")]
        #FeatureDF["name"]=FeatureDF.index
        FeatureDF.index.name="name"
        FeatureDF["chr"]=FeatureDF.apply(lambda x: x["genomic_interval"].chrom, axis=1)
        FeatureDF["strand"]=FeatureDF.apply(lambda x: x["genomic_interval"].strand, axis=1)
        FeatureDF["start"]=FeatureDF.apply(lambda x: x["genomic_interval"].start_d_as_pos.start, axis=1)
        FeatureDF["end"]=FeatureDF.apply(lambda x: x["genomic_interval"].start_d_as_pos.start, axis=1)
        Feature_Annotation = FeatureDF
    for en in analysis.getExperimentList():
    ...
    for genotype in ("wt", "ko"):
        ...
        >>> analysis.Annotation.columns
        Index([u'reference', u'genomic_interval', u'variants', u'exons',
               u'exon_specific', u'introns_all'],
              dtype='object')
        >>> type(analysis.Annotation["genomic_interval"][10])
        <type 'HTSeq._HTSeq.GenomicInterval'>
        >>> analysis.Annotation.loc["Zyx","genomic_interval"]
        <GenomicInterval object 'chr6', [42349827,42360213), strand '+'>
        >>> analysis.Annotation[analysis.Annotation.index.isin(["Zyx","Zscan4a"])]
                                    reference  ...                                        introns_all
        genes                                  ...                                                   
        Zscan4a                       [13084]  ...  [chr7:[10795781,10795865)/-, chr7:[10796025,10...
        Zyx      [11276, 11278, 11279, 11284]  ...  [chr6:[42350119,42350256)/+, chr6:[42350476,42...

        [2 rows x 6 columns]
                                                                                       
        annotDF = pd.DataFrame(columns= ["name", "chr", "start", "end", "strand"])
        annotDF[name]=[]
        featDF=annotDF[annotDF["name"].isin([2,3])] use isin to get gene list from annotation DF
        """
...
"""                        
                        (ANNOTATION_FILE_PATH + "topGenes_TSS.csv", "topGenes"),     # expression level groups
                        (ANNOTATION_FILE_PATH + "flopGenes_TSS.csv", "flopGenes"),
                        (ANNOTATION_FILE_PATH + "ES_genes_high.csv", "highGenes"),
                        (ANNOTATION_FILE_PATH + "ES_genes_medium.csv", "mediumGenes"),
                        (ANNOTATION_FILE_PATH + "ES_genes_low.csv", "lowGenes"),

                        (ANNOTATION_FILE_PATH + "ES_K27_Genes.csv", "ES_K27_Genes"), # K4, K27 and bivalent genes
...
def plotK4K27GeneGroups():
    Heizmappe(analysis, ANNOTATION_FILE_PATH + "ES_K27_Genes.csv", "ES_K27_Genes", interval=(-5000,5000), window=100, chromosome="chrX", z_transform=False, sort_rows=True, descending=True, log_color_range=True, xtickrotation=90, cbar=True)
    # no K4K27 genes on chrX -- Heizmappe(analysis, ANNOTATION_FILE_PATH + "ES_K4K27_Genes.csv", "ES_K4K27_Genes", interval=(-5000,5000), window=100, chromosome="chrX", z_transform=False, sort_rows=True, descending=True, log_color_range=True, xtickrotation=90, cbar=True)
    Heizmappe(analysis, ANNOTATION_FILE_PATH + "ES_K4_Genes.csv", "ES_K4_Genes", interval=(-5000,5000), window=100, chromosome="chrX", z_transform=False, sort_rows=True, descending=True, log_color_range=True, xtickrotation=90, cbar=True)
...
PERFORM_EXTENSIVE_ANALYSIS = False # ----------------- True
...
if ATACSEQ_ANALYSIS:
    ...
    if LOAD_CURRENT_ANALYSIS:
        ...
        if LOAD_ALL_ATACSEQ_ANALYSIS:
            ...
            samples = {"wt": ATAC_wt_samples, "ko": ATAC_ko_samples}
            ATACseq_experiments = { "WT1": "../../ATACSec/ATACseq_analysis_q10/replicate_1/WT1.wig", "WT1_DOX": "../../ATACSec/ATACseq_analysis_q10/replicate_1/WT1_DOX.wig", "KO1": "../../ATACSec/ATACseq_analysis_q10/replicate_1/KO1.wig", "KO1_DOX": "../../ATACSec/ATACseq_analysis_q10/replicate_1/KO1_DOX.wig",
                                    "WT2": "../../ATACSec/ATACseq_analysis_q10/replicate_2/WT2.wig", "WT2_DOX": "../../ATACSec/ATACseq_analysis_q10/replicate_2/WT2_DOX.wig", "KO2": "../../ATACSec/ATACseq_analysis_q10/replicate_2/KO2.wig", "KO2_DOX": "../../ATACSec/ATACseq_analysis_q10/replicate_2/KO2_DOX.wig",
                                    "WT3": "../../ATACSec/ATACseq_analysis_q10/replicate_3/WT3.wig", "WT3_DOX": "../../ATACSec/ATACseq_analysis_q10/replicate_3/WT3_DOX.wig", "KO3": "../../ATACSec/ATACseq_analysis_q10/replicate_3/KO3.wig", "KO3_DOX": "../../ATACSec/ATACseq_analysis_q10/replicate_3/KO3_DOX.wig" }
            ATACSEQ_DATA_FILE_PATH="../../ATACSeq/ATACSeq_analysis_q10/"
            DATA_FILE_PATH = ATACSEQ_DATA_FILE_PATH
            analysis = XchipAnalysis()

            inFile = open('../../ATACSeq/Annotation', "rb")               # load annotation pickel file
            # this is system and version dependent and might require generating a HaSAPPy annotation to be generated on a particular computer system
            ...
            for expStatFolder in ["../../ATACSec/ATACseq_analysis_q10/replicate_1/", "../../ATACSec/ATACseq_analysis_q10/replicate_2/", "../../ATACSec/ATACseq_analysis_q10/replicate_3/" ]:
            ...
            # analysis.saveAnalysis(ATACSEQ_DATA_FILE_PATH) # <------------------------------------------------- TO SAVE A NEWLY CALCULATED ANALYSIS UNCOMMENT
    ...
    if PERFORM_EXTENSIVE_ANALYSIS:
        ...
        plotK4K27GeneGroups() # ------------------------------ else clause to get K4 prof heatmap
